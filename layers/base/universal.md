...
- Use descriptive, unambiguous names. A name like "remaining attempts" not "cnt", "customer email address" not "email". Follow your language's casing convention for identifiers.
- Do not abbreviate unless the abbreviation is universally understood in the domain (e.g., `id`, `url`, `http`).
- Booleans use affirmative prefixes: `is`, `has`, `can`. Never negate the name — use the positive form and negate at the call site.
- Collections use plural nouns: `customers`, `orderItems` / `order_items`, `activeSubscriptions` / `active_subscriptions`.
...
All code for a business feature lives in one directory: request handlers, business logic, data access, request/response models, mapping logic, errors, and tests. This is non-negotiable.

```
src/
├── billing/          # Everything for billing: handlers, service, data access, models, errors
...
- Maintain a small set of categories: **not found**, **validation**, **business rule violation**, **authentication**, **authorisation**, **upstream/integration failure**. Unhandled errors must never expose internals.
...
Comments explain **why**, not what. If code needs a comment to explain what it does, refactor it. Mark `HACK`, `FIXME`, and `TODO` annotations with a ticket reference — never without one. Every project has a README covering: what it is, local setup, testing, deployment, and key architectural decisions.

## Security

- Validate all input at system boundaries: type, format, length, range. Apply business rule validation in the service layer after sanitisation.
- Never construct structured output (SQL, HTML, XML, JSON, etc.) by string concatenation. Use parameterised queries, template engines, and serialisation libraries.
- Use established authentication standards and identity providers. Never implement custom authentication or token handling.
- Encrypt sensitive data at rest. Least privilege for services and database users. Vulnerability scans in CI. Pin dependency versions — no dynamic ranges in production.
